...
In soem cases, it may be desireable to give priority to the readers so they can make updates and prevent readers from accessing old data. To create the writer priority, two switches are utilized: one for the readers and one for the writers. When a writer is in the critical section, it allows only other writers to queue which gives them priority.
